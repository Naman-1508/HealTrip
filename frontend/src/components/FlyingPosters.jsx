...
class Media {
  constructor({ gl, geometry, scene, screen, viewport, image, length, index, planeWidth, planeHeight, distortion }) {
    ...
    this.length = length;
    this.index = index;
    this.planeWidth = planeWidth; // Actually width of the plane
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uSpeed: { value: 0 },
        rotationAxis: { value: [0, 1, 0] }, // Rotate around Y axis (standard for vertical distortion), maybe Z for horizontal flow?
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.padding = 0.5; // Padding between items
    // HORIZONTAL Logic:
    this.width = this.plane.scale.x + this.padding;
    this.widthTotal = this.width * this.length;

    // Center horizontally adjusted by index
...
class Canvas {
  ...
  onTouchMove(e) {
    ...
    const distance = (this.start - x) * 0.01; // Slower touch scroll
    this.scroll.target = this.scroll.position + distance * 10;
  }
...
export default function FlyingPosters({
  items = [],
  planeWidth = 400, // Wider for horizontal cards
  planeHeight = 250,
  distortion = 2,
  scrollEase = 0.05,
  cameraFov = 45,
