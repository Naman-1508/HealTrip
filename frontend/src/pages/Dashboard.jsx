...
import { User, Activity, HeartPulse, Map, Hospital, Stethoscope, Briefcase, Calendar, CheckCircle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/payment/my-bookings', {
        headers: {
  ...
  if (!isLoaded) return <div className="pt-28 text-center text-white">Loading...</div>;
  if (!user) { navigate("/login"); return null; }

  // Filter Active Packages
  const activePackages = bookings.filter(b => b.status !== 'cancelled' && b.status !== 'completed');
  
  const stats = [
    { label: "Bookings", value: bookings.length, icon: Briefcase, color: "text-purple-500" },
    { label: "Active Plans", value: activePackages.length, icon: Map, color: "text-blue-500" },
    { label: "Health Score", value: "Good", icon: Activity, color: "text-green-500" },
  ];
  ...
  return (
    <div className="pt-28 px-8 pb-20 min-h-screen bg-zinc-950 text-white">
      ...
      >
        <div>
          <h1 className="text-4xl font-heading font-bold text-white">Dashboard</h1>
      ...
      {activePackages.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {activePackages.map((booking) => {
             ...
             // Dynamic Data
             const title = isYoga ? booking.data.Center_Name 
                           : isWellness ? booking.data.Session_Name 
                           : booking.hospital?.name || "Hospital Visit";
             
             const subtext = isYoga ? `${booking.data.Yoga_Style} • ${booking.data.City}`
                             : isWellness ? `${booking.data.Session_Type} • ${booking.data.City}`
                             : `${booking.hotel?.name || "No Hotel"} • ${booking.packageDetails?.duration || 1} Days`;
             
             const icon = isYoga ? <User className="text-purple-400" /> 
                          : isWellness ? <HeartPulse className="text-blue-400" />
                          : <Hospital className="text-emerald-400" />;

             const bgColor = isYoga ? "bg-purple-900/20 border-purple-500/30"
                             : isWellness ? "bg-blue-900/20 border-blue-500/30"
             ...
             return (
                ...
                >
                  <div className={`h-32 relative ${isWellness ? 'bg-zinc-800/50' : 'bg-zinc-800'}`}>
                    <div className="absolute inset-0 flex items-center justify-center text-zinc-600">
                      {isYoga ? <User size={48} opacity={0.3} /> : isWellness ? <HeartPulse size={48} opacity={0.3} /> : <Map size={48} opacity={0.2} />}
                    </div>
                    <div className={`absolute top-4 right-4 text-black text-xs font-bold px-2 py-1 rounded-full uppercase ${
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-white/10">
                      ...
                      <span className="text-lg font-bold text-emerald-400">₹{booking.pricing?.total?.toLocaleString()}</span>
                    </div>
                  </div>
                </motion.div>
             );
        ...
        <div className="bg-zinc-900/30 border border-white/5 rounded-2xl p-8 text-center mb-12">
          ...
          <button onClick={() => navigate('/travel')} className="text-emerald-400 hover:underline">Start planning a trip</button>
        </div>
      )}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              {bookings.length > 0 ? bookings.map((b) => {
                 ...
                 const itemName = isYoga ? b.data.Center_Name : isWellness ? b.data.Session_Name : b.hospital?.name;
                 const itemSub = isYoga ? "Yoga Session" : isWellness ? "Mental Wellness" : (b.hotel?.name || "Hotel Booking");
                    ...
                    <tr key={b._id} className="hover:bg-white/5 transition">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-zinc-500 text-xs">{itemSub}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${
                          b.status === 'completed' || b.status === 'confirmed' ? 'bg-emerald-500/20 text-emerald-400' :
