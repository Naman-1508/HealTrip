      ...
      if (response.ok) {
        ...
        if (data && data.length > 0) {
          ...
          const mappedFlights = data.map((f, i) => ({
            ...
            arrivalTime: new Date().setHours(
              8 + i + f.duration_minutes / 60,
  ...
  const applyFilters = () => {
    ...
    if (flightFilters.maxPrice) {
      filteredFlights = filteredFlights.filter(
        (f) => f.price <= parseFloat(flightFilters.maxPrice),
      );
    }
    if (flightFilters.airline) {
      filteredFlights = filteredFlights.filter((f) =>
        f.airline.toLowerCase().includes(flightFilters.airline.toLowerCase()),
      );
    }
    ...
    if (hotelFilters.minRating) {
      filteredHotels = filteredHotels.filter(
        (h) => h.rating >= parseFloat(hotelFilters.minRating),
      );
    }
    if (hotelFilters.maxPrice) {
      filteredHotels = filteredHotels.filter(
        (h) => h.pricePerNight <= parseFloat(hotelFilters.maxPrice),
      );
    }
    ...
    if (hospitalFilters.specialty) {
      filteredHospitals = filteredHospitals.filter((h) =>
        h.specialty
          .toLowerCase()
          .includes(hospitalFilters.specialty.toLowerCase()),
      );
    }
    if (hospitalFilters.minRating) {
      filteredHospitals = filteredHospitals.filter(
        (h) => h.rating >= parseFloat(hospitalFilters.minRating),
      );
    }
              ...
              {flights.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {flights.map((f, i) => (
                    <div
                      key={i}
                      className="bg-zinc-800/50 p-6 rounded-xl border border-white/5 flex justify-between items-center hover:bg-zinc-800 transition"
                    >
                      <div>
              ...
              {!loadingHotels && hotels.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {hotels.map((h, i) => (
                    <div
                      key={i}
                      className="bg-zinc-800 rounded-xl overflow-hidden border border-white/5 group"
                    >
                      <div className="h-48 overflow-hidden relative">
                        ...
                        />
                        <div className="absolute top-2 right-2 bg-black/60 px-2 py-1 rounded text-xs text-violet-400 font-bold">
                          ★ {h.rating}
                        </div>
                      </div>
                      <div className="p-4">
                        ...
                        <div className="flex flex-wrap gap-1 my-3">
                          {h.amenities.slice(0, 3).map((a, idx) => (
                            <span
                              key={idx}
                              className="text-[10px] bg-zinc-700 px-2 py-1 rounded-full"
                            >
                              {a}
                            </span>
                          ))}
              ...
              {visaResult && (
                <div className="bg-zinc-800/50 rounded-xl p-6 border border-white/5 mt-6">
                  <h3 className="text-xl font-bold text-orange-400 mb-4">
                    Visa Requirements for {visaResult.country}
                  </h3>
                    ...
                    <div>
                      ...
                      <div className="mt-4 p-3 bg-orange-900/20 rounded-lg border border-orange-500/20">
                        <p className="text-xs text-orange-300">
                          Processing Time: {visaResult.processing_time}
                          <br />
              ...
              {!loadingHospitals && hospitals.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {hospitals.map((h, i) => (
                    <div
                      key={i}
                      className="bg-zinc-800 rounded-xl overflow-hidden border border-white/5 flex flex-col"
                    >
                      <img
                        src={h.images[0]}
                        alt={h.name}
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <p className="text-xs text-emerald-300 mb-4 bg-emerald-900/30 px-2 py-1 rounded inline-block">
                          {h.specialty}
                        </p>
                ...
                !loadingHospitals && (
                  <div className="text-center py-10 text-zinc-500">
                    Enter a medical condition to see AI-ranked hospital
                    recommendations
                  </div>
                )
              )}
