...
export default function Hospitals() {
  ...
  const specialties = [
    ...
    "Pediatrics", "Dermatology", "Gastroenterology", "Ophthalmology"
  ];
  ...
  const fetchHospitals = async () => {
    ...
    try {
      ...
      if (specialty && specialty !== "All") backendUrl += `&treatment=${specialty}`;
      promises.push(axios.get(backendUrl).catch(e => ({ data: { success: false } })));

      // 2. ML Search (City-based) - Only if search query exists
      if (debouncedSearch && debouncedSearch.length > 2) {
         promises.push(axios.get(`http://localhost:8001/hospitals-by-city?city=${debouncedSearch}`).catch(e => ({ data: [] })));
      }

      const results = await Promise.all(promises);
      const backendData = results[0].data?.success ? results[0].data.data.hospitals : [];
      const mlData = results[1]?.data || [];

      // Transform ML Data to match Frontend Schema
      const mlHospitals = Array.isArray(mlData) ? mlData.map((h, i) => ({
          ...
          description: h.summary || "Top rated hospital in " + (h.city || "India")
      })) : [];

      // Merge: Prioritize ML results for City searches
      // Deduplicate by name
      const allHospitals = [...mlHospitals, ...backendData];
      const uniqueHospitals = Array.from(new Map(allHospitals.map(item => [item.name, item])).values());
  ...
  useEffect(() => {
    if (search.length > 2) {
      const fetchSuggestions = async () => {
        ...
        try {
          const promises = [
            axios.get(`http://localhost:5000/api/hospitals?search=${search}&limit=5`).catch(e => ({ data: { success: false } })), // Backend
            axios.get(`http://localhost:8001/hospitals-by-city?city=${search}`).catch(e => ({ data: [] })) // ML
          ];

          const results = await Promise.all(promises);
          const backendSugg = results[0].data?.success ? results[0].data.data.hospitals : [];
          const mlSuggRaw = results[1]?.data || [];

          // Transform ML Suggestions
          const mlSugg = Array.isArray(mlSuggRaw) ? mlSuggRaw.slice(0, 5).map((h, i) => ({
             _id: `ml_sugg_${i}`,
             name: h.name,
          ...
          const unique = Array.from(new Map(combined.map(item => [item.name, item])).values()).slice(0, 5);
  ...
  if (hospitals.length > 0) {
      galleryItems = hospitals.slice(0, 15).map((h, i) => ({
          image: h.images?.[0]?.url || h.image || defaultImages[i % defaultImages.length],
          text: h.name, 
      ...
      <div className="relative z-10 w-full pt-28 mb-10">
         <div className="text-center mb-8 px-6">
             ...
             <p className="text-zinc-400 mt-2 text-lg max-w-2xl mx-auto">
                 Explore top-tier medical facilities worldwide through our curated 3D showcase.
             </p>
         </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-10 bg-zinc-900/50 p-6 rounded-2xl border border-white/5 backdrop-blur-xl shadow-2xl z-50 relative">
                ...
                {loadingSuggestions ? (
                  ...
                  suggestions.map((s) => (
                    ...
                    >
                      ...
                      />
                      <div>
                        <h4 className="text-sm font-bold text-white max-w-[200px] truncate">{s.name}</h4>
          ...
          <div className="relative w-full md:w-64">
            ...
            >
              ...
              {specialties.map(s => (
                <option key={s} value={s} className="bg-zinc-900">{s} Specialty</option>
              ))}
            </select>
          </div>
        </div>
                  ...
                  <div className="flex flex-col h-full">
                    <div className="h-40 w-full rounded-lg overflow-hidden mb-4 relative">
                        {(h.images && h.images.length > 0 ? (
                          <img src={h.images[0].url || h.images[0]} alt={h.name} className="w-full h-full object-cover transform hover:scale-105 transition duration-700" />
                        ) : (
                          <img src={h.image || "https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d"} alt={h.name} className="w-full h-full object-cover" />
                        ))}
                        <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md px-2 py-1 rounded text-xs font-bold text-white flex items-center gap-1">
                            <span className="text-yellow-400">★</span> {h.rating ? h.rating.toFixed(1) : 4.5}
                        </div>
                    </div>
                    
                    <h3 className="font-heading text-lg font-bold text-white truncate">{h.name}</h3>
                    <p className="text-sm text-zinc-400 mb-3 truncate">{addressStr}</p>

                    <div className="flex flex-wrap gap-1 mb-4">
                      {h.specialties && Array.isArray(h.specialties) && h.specialties.slice(0, 3).map((spec, i) => (
                        <span key={i} className="text-[10px] bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 px-2 py-1 rounded-full">
                           {typeof spec === 'object' ? (spec.name || 'Specialty') : spec}
                        </span>
                      ))}
                    </div>
