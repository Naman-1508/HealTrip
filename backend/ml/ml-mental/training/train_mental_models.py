...
def synthesize_mental_features(df):
    ...
    topics_list = ['Anxiety', 'Depression', 'Stress', 'Relationships', 'Trauma', 'Career', 'Addiction']
    amenities_list = ['Private Room', 'Online Option', 'Refreshments', 'Wheelchair Access', 'Insurance Accepted']
    ...
    if 'Session_Type' not in df.columns:
        ...
        df[['Session_Type', 'Topics_Covered', 'Amenities', 'Fee']] = df.apply(get_features, axis=1)
    return df
...
def preprocess_mental(df):
    ...
    df['Topics_Count'] = df['Topics_Covered'].apply(lambda x: len(x.split(',')) if x else 0)
    df['combined_text'] = (
        df.get('City', '') + " " + 
        df.get('Session_Type', '') + " " + 
...
def train_and_save():
    ...
    with open(os.path.join(MODELS_DIR, 'mental_vectorizer.pkl'), 'wb') as f: pickle.dump(tfidf, f)
    with open(os.path.join(MODELS_DIR, 'mental_tfidf_matrix.pkl'), 'wb') as f: pickle.dump(tfidf_matrix, f)
    ...
    with open(os.path.join(MODELS_DIR, 'mental_price_model.pkl'), 'wb') as f: pickle.dump(model_fee, f)
        
    # 3. Clustering
    print("Training Clustering Model...")
    scaler = StandardScaler()
    features_cluster = scaler.fit_transform(df[['Fee', 'Amenities_Count', 'Topics_Count']])
    ...
    with open(os.path.join(MODELS_DIR, 'encoders.pkl'), 'wb') as f: pickle.dump(encoders, f)
