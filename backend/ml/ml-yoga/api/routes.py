...
def load_models():
    ...
    try:
        with open(os.path.join(MODELS_DIR, 'yoga_vectorizer.pkl'), 'rb') as f: models['vec'] = pickle.load(f)
        with open(os.path.join(MODELS_DIR, 'yoga_tfidf_matrix.pkl'), 'rb') as f: models['mat'] = pickle.load(f)
        with open(os.path.join(MODELS_DIR, 'yoga_price_model.pkl'), 'rb') as f: models['price'] = pickle.load(f)
...
def get_yoga():
    if 'yoga' not in data: raise HTTPException(503, "Not loaded")
...
def rec_yoga(city: str, focus: str, budget: float = None):
    ...
    df = data['yoga'].copy()
    df['sim'] = sim
    if budget: df = df[df['Price'] <= budget]
    return df.sort_values(by=['sim', 'Price'], ascending=[False, True]).head(10).replace({np.nan: None}).to_dict(orient='records')
...
def get_yoga_cluster(session_title: str):
    if 'yoga' not in data: raise HTTPException(503, "Not loaded")
    match = data['yoga'][data['yoga']['Center_Name'].str.contains(session_title, case=False, na=False)]
