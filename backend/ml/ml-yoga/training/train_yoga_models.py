...
def synthesize_yoga_features(df):
    ...
    focuses = ['Meditation', 'Strength', 'Flexibility', 'Stress Relief', 'Weight Loss', 'Mindfulness']
    amenities_list = ['Mats', 'Showers', 'AC', 'Parking', 'Locker', 'Juice Bar', 'Spa', 'Pool']
    ...
    if 'Yoga_Style' not in df.columns:
        ...
        df[['Yoga_Style', 'Session_Focus', 'Amenities', 'Price']] = df.apply(get_features, axis=1)
...
def train_and_save():
    ...
    with open(os.path.join(MODELS_DIR, 'yoga_vectorizer.pkl'), 'wb') as f: pickle.dump(tfidf, f)
    with open(os.path.join(MODELS_DIR, 'yoga_tfidf_matrix.pkl'), 'wb') as f: pickle.dump(tfidf_matrix, f)
    ...
    with open(os.path.join(MODELS_DIR, 'yoga_price_model.pkl'), 'wb') as f: pickle.dump(model_price, f)
